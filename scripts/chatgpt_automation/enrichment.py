...
def build_multimodal_description_prompt(annotation: dict[str, Any], language: str = "en") -> str:
    ...
    return (
        ...
        "- name annotation types precisely: text labels, price labels, horizontal/vertical lines, trendlines, current-price lines, arrows, swing markers, range boxes, session boxes, supply/demand zones\n"
        "- identify trading-platform drawing tools by function when visible, especially long/short position or risk/reward tools; do not reduce them to generic rectangles\n"
        "- distinguish session/range overlays from trade planning tools with entry, stop-loss/risk, take-profit/reward, CRV, R:R, or diagonal connector lines\n"
